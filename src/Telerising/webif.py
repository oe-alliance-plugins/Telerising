...
def setupWeb(*args):
	if not Path(resolveFilename(SCOPE_PLUGINS, "Extensions/OpenWebif/pluginshook.src")).is_file():
		return
	configuration = getattr(config, "OpenWebif", None)
	if configuration and setupWeb not in configuration.enabled.notifiersFinal:
		configuration.enabled.addNotifier(setupWeb, initial_call=False, immediate_feedback=False)
	try:
		# OpenWebif creates this compatibility module on its first server start.
		hook = import_module("Plugins.Extensions.WebInterface.WebChilds.Toplevel")
	except ImportError:
		return
	for index, entry in enumerate(hook.loaded_plugins):
		if entry[0] == "telerising":
			resource = entry[1]
			hook.loaded_plugins[index] = ("telerising", resource, "Telerising", __version__, settings.webif.value, "_self")
			break
	else:
		resource = TelerisingResource()
		hook.addExternalChild(("telerising", resource, "Telerising", __version__, settings.webif.value, "_self"))
	# addExternalChild only updates the registry. Attach to running HTTP/HTTPS
	# roots too, behind OpenWebif's existing authentication wrapper.
	httpserver = modules.get("Plugins.Extensions.OpenWebif.httpserver")
	for name in ("site", "sslsite"):
		site = getattr(httpserver, name, None)
		if site is not None:
			site.resource.resource.putChild2("telerising", resource)
